...
export const handler: CloudFrontRequestHandler = async (event: CloudFrontRequestEvent) =>
  AsyncHandler.handleAsync(event, {
    region: defaultRegion,
    logLevel: 'info',
    userPoolIdResolver: async () =>
      (await ssmClient.send(new GetParameterCommand({ Name: ssmParamNames.userPoolIdParamName })))
        .Parameter?.Value || '',
    userPoolClientIdResolver: async () =>
      (
        await ssmClient.send(
          new GetParameterCommand({ Name: ssmParamNames.userPoolClientIdParamName })
        )
      ).Parameter?.Value || '',
    userPoolDomainResolver: async () =>
      (
        await ssmClient.send(
          new GetParameterCommand({ Name: ssmParamNames.userPoolDomainParamName })
        )
      ).Parameter?.Value || ''
  })
